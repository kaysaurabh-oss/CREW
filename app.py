...
# Built-in access accounts are repaired on every startup so a previously-created
# database cannot lock out admin/office during a pilot run. Change these before
# live deployment.
BUILTIN_USERS = [
...
def ensure_builtin_users(con):
    """Keep pilot admin/office accounts usable even if the DB was already created.

    This avoids first-run seeding problems where users.csv was corrected after
    sire_campaign.db had already been created. Vessel users are not overwritten.
    """
    cur = con.cursor()
    for username, password, role, vessel_id, display_name in BUILTIN_USERS:
        cur.execute(
            """
            INSERT INTO users(username,password_hash,role,vessel_id,display_name,active)
            VALUES(?,?,?,?,?,1)
            ON CONFLICT(username) DO UPDATE SET
                ...
                active=1
            """,
            (username, hash_password(password), role, vessel_id, display_name),
        )
    con.commit()
    ...
    if USERS_CSV.exists():
        ...
        for _, r in users.iterrows():
            ...
            if not username or username in {"admin", "office"}:
                continue
            rows.append((username, hash_password(str(r["password"]).strip()), r["role"], r.get("vessel_id", ""), r.get("display_name", username), 1))
        cur.executemany(
            """
            INSERT INTO users(username,password_hash,role,vessel_id,display_name,active)
            VALUES(?,?,?,?,?,?)
...
def login_box():
    ...
    with c1:
        ...
        if st.button("Login", type="primary", use_container_width=True):
            ...
            con = connect()
            row = con.execute("SELECT * FROM users WHERE username=? AND active=1", (clean_username,)).fetchone()
            con.close()
            if row and verify_password(clean_password, row["password_hash"]):
